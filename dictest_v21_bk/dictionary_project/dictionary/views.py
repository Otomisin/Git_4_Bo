...
def home(request):
    query = request.GET.get('q', '').strip()
    if query:
        # Remove the word_source filter
        word = Word.objects.filter(word__iexact=query).order_by('word').first()
        if word:
            return redirect('word_detail', pk=word.pk)
        return render(request, 'dictionary/home.html', {
            'show_hero': True,
...
def search_suggestions(request):
    ...
    if query:
        # Remove the word_source filter
        suggestions = Word.objects.filter(word__istartswith=query).order_by(
            'word').values('word', 'definition')[:9]
        formatted_suggestions = [
            {
                "word": item['word'].title(),
                "html": f"<strong>{item['word'].title()}</strong>: <span style='font-style: italic; font-size: 0.85em;'>{item['definition']}</span>"
